import React from 'react'

export const MoreDetailsACRepair = () => {
  return (
    <div className='all_details'>
        <div className='title_why_ser'>
            <div><i className='caret right icon'></i>Water Leakage</div>
            <div><i className='caret right icon'></i>Power on issue</div>
            <div><i className='caret right icon'></i>Less Cooling / No Cooling</div>
            <div><i className='caret right icon'></i>Unwanted Noise / Smell</div>
        </div>

        <div className='more'>
            <div className='mid_name'>What's Included</div>
            <div><i className='caret right icon'></i>
                Get upto 6 months Warrenty
            </div>
            
            
            <div><i className='caret right icon'></i>
                Adjustment of inspection fee (Rs. 399) in invoice, if
                Eagle Refrigeration repair services is availed.
            </div>
            <div><i className='caret right icon'></i>
                AC repair using high-quality spare parts and tools.
            </div>
            <div><i className='caret right icon'></i>
                Final checks and clean up.
            </div>
        

            <div className='note_ser'>
            
                Note-
                Ladder is not carried by Technicians.
                Please manage if AC is at a height.
            
            </div>
        </div>
    </div>
  )
}
